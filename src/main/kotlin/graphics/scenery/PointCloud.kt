...
open class PointCloud(var pointRadius: Float = 1.0f, override var name: String = "PointCloud") : Mesh(name) {
    ...
    fun setupPointCloud() {
        geometry {
            if( texcoords.limit() == 0 ) {// Only preinitialize if texcoords has not been preinialized
                texcoords = BufferUtils.allocateFloat(vertices.limit()/3 * 2)
                var i = 0
                while (i < texcoords.limit() - 1) {
                    texcoords.put(i, pointRadius)
                    texcoords.put(i + 1, pointRadius)
                    i += 2
                }
            }
            if( normals.limit() == 0 ) {// Only preinitialize if need be
                normals = BufferUtils.allocateFloatAndPut( FloatArray(vertices.limit(), { 1.0f } ) )
            }
        }
    }
    ...
    fun readFromPALM(filename: String) {
        ...
        geometry {
            ...
            vertices = BufferUtils.allocateFloat(count.toInt()*3)
            normals = BufferUtils.allocateFloat(count.toInt()*3)
            texcoords = BufferUtils.allocateFloat(count.toInt()*2)
            ...
            Files.lines(FileSystems.getDefault().getPath(filename)).forEach {
                ...
                if (!arr[0].startsWith("\"")) {
                    vertices.put(arr[1].toFloat())
                    vertices.put(arr[2].toFloat())
                    vertices.put(arr[3].toFloat())
                    ...
                    normals.put(arr[1].toFloat())
                    normals.put(arr[2].toFloat())
                    normals.put(arr[3].toFloat())

                    texcoords.put(arr[4].toFloat())
                    texcoords.put(arr[5].toFloat())
                }
            }

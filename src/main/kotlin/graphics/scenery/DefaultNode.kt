...
open class DefaultNode(override var name: String) : Node {

    private val properties = HashMap<Class<*>, Any>()

    final override fun <T> addProperties(propertiesType: Class<T>, props: T) {
        this.properties.put(propertiesType, props as Any)
    }

    final override fun <T> getProperties(propertiesType: Class<T>, block: T.() -> Unit) : T? {
        val props = properties.get(propertiesType) ?: return null
    ...
    override fun getMaximumBoundingBox(): OrientedBoundingBox {
        ...
        return children
            .filter { it !is BoundingGrid  }.map { it.getMaximumBoundingBox().translate(it.spatial()?.position ?: Vector3f(0f, 0f, 0f)) }

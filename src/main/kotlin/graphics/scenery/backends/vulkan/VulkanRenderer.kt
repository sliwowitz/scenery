...
//            .parallelMap(numThreads = System.getProperty("scenery.MaxInitThreads", "1").toInt()) { node ->
        ...
        scene.renderable().initialized = true
    ...
    fun destroyNode(node: Node) {
        ...
        lateResizeInitializers.remove(node.renderable())
...
//        profiler?.begin("Renderer.Housekeeping")
        ...
        if (scene.children.count() == 0 || !scene.renderable().initialized) {
            initializeScene()
...
//        scene.activeObserver?.deltaT = duration / 10E6f
    ...
    private fun updateDefaultUBOs(device: VulkanDevice, cam: Camera): Boolean = runBlocking {
        ...
        if (!cam.renderable().lock.tryLock()) {
            return@runBlocking false
        }
        ...
        camSpatial.view = cam.getTransformation()
//        camSpatial.updateWorld(true, false)
...
//        updateDescriptorSets()
    ...
    fun closeInternal() {
        ...
        scene.renderable().initialized = false

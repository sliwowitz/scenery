...
open class VulkanFramebuffer(protected val device: VulkanDevice,
    ...
    inner class VulkanFramebufferAttachment : AutoCloseable {
         ...
         */
        override fun close() {
            vkDev.destroyImageView(imageView)

            if (image.isValid && !fromSwapchain) {
                vkDev.destroyImage(image)
            }
    ...
    protected fun createAttachment(format: VkFormat, usage: VkImageUsage, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebufferAttachment {
        ...
        var aspectMask = VkImageAspect(0)
        ...
        if (usage == VkImageUsage.DEPTH_STENCIL_ATTACHMENT_BIT) {
            aspectMask = VkImageAspect.DEPTH_BIT
            imageLayout = VkImageLayout.SHADER_READ_ONLY_OPTIMAL
        }
    ...
    fun addFloatBuffer(name: String, channelDepth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (channelDepth) {
            ...
            else -> VkFormat.R16_SFLOAT.also { logger.warn("Unsupported channel depth $channelDepth, using 16 bit.") }
        }
    ...
    fun addFloatRGBuffer(name: String, channelDepth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (channelDepth) {
            ...
            else -> VkFormat.R16G16_SFLOAT.also { logger.warn("Unsupported channel depth $channelDepth, using 16 bit.") }
        }
    ...
    fun addFloatRGBBuffer(name: String, channelDepth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (channelDepth) {
            ...
            else -> VkFormat.R16G16B16A16_SFLOAT.also { logger.warn("Unsupported channel depth $channelDepth, using 16 bit.") }
        }
    ...
    fun addUnsignedByteRGBABuffer(name: String, channelDepth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (channelDepth) {
            ...
            else -> VkFormat.R16G16B16A16_UINT.also { logger.warn("Unsupported channel depth $channelDepth, using 16 bit.") }
        }
    ...
    fun addUnsignedByteRBuffer(name: String, channelDepth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (channelDepth) {
            ...
            else -> VkFormat.R16_UNORM.also { logger.warn("Unsupported channel depth $channelDepth, using 16 bit.") }
        }
    ...
    fun addDepthBuffer(name: String, depth: Int, attachmentWidth: Int = width, attachmentHeight: Int = height): VulkanFramebuffer {
        val format = when (depth) {
            ...
            else -> VkFormat.D32_SFLOAT.also { logger.warn("Unsupported channel depth $depth, using 32 bit.") }
        }
    ...
    fun addSwapchainAttachment(name: String, swapchain: Swapchain, index: Int): VulkanFramebuffer {
        val att = VulkanFramebufferAttachment().apply {
            image = VkImage(swapchain.images[index])
            imageView = VkImageView(swapchain.imageViews[index])
    ...
    protected fun getAttachmentDescBuffer(): VkAttachmentDescription.Buffer {
        return VkAttachmentDescription.calloc(attachments.size).fill(attachments.values.map { it.desc })
    }
    ...
    fun createRenderpassAndFramebuffer() {
        ...
        colorDescs.forEachIndexed { i, it ->
            it.attachment = i
            it.layout = VkImageLayout.COLOR_ATTACHMENT_OPTIMAL
        }

        val depthDescs: VkAttachmentReference? = when {
            attachments.any { it.value.type == VulkanFramebufferType.DEPTH_ATTACHMENT } -> vk.AttachmentReference {
        ...
        getAttachmentDescBuffer().use { attachmentDescs ->
        ...
        getAttachmentImageViews().use { attachmentImageViews ->
            val fbinfo = vk.FramebufferCreateInfo().also {
                it.renderPass = renderPass
                it.attachments = attachmentImageViews
                it.width = width
                it.height = height
                it.layers = 1
            }
    ...
    override fun close() {
        if (initialized) {
            ...
            vkDev.destroyRenderPass(renderPass)
            vkDev.destroySampler(framebufferSampler)
            vkDev.destroyFramebuffer(framebuffer)

...
class ComputeShaderExample : SceneryBase("ComputeShaderExample") {
    override fun init() {
        ...
        compute.renderable {
            material = ShaderMaterial(Shaders.ShadersFromFiles(arrayOf("BGRAMosaic.comp"), this@ComputeShaderExample::class.java))
            material.textures["OutputViewport"] = Texture.fromImage(Image(buffer, helix.dimensions.x, helix.dimensions.y, helix.dimensions.z), usage = hashSetOf(Texture.UsageType.LoadStoreImage, Texture.UsageType.Texture))
            material.textures["InputColor"] = helix
        }
        ...
        box.renderable {
            material.textures["diffuse"] = compute.renderable().material.textures["OutputViewport"]!!
